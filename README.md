...
```powershell
.\backend\.venv\Scripts\python.exe .\backend\scripts\import_microbiology.py .\microbiology_cohort_deid_tj_updated.csv --install
```

預設搜尋文件庫全部真實參考文件（`evidence_scope: reference`、`policy_refs: []`），不混入合成測試文件。可加 `--policy-ref 文件ID` 固定指定文件；也可在病例工作台切換全部參考文件或勾選指定文件。`--per-site 4` 是預設，每種檢體最多選 4 個來源培養，再新增 3 個配對情境。CSV 全檔採串流掃描，兩次通讀以完整收集不相鄰的藥敏列，不把 496 萬列都當成病人匯入資料庫。
...
來源病例預設可用 rule-only、mock 或本機 Ollama。若來源資料與文件使用條款允許外送，可在病例摘要勾選「允許此病例的必要分析欄位與所選文件片段送至外部模型」，會保存新版本；之後選 live 才會呼叫已設定的 OpenAI 等服務。傳送資料不含來源病人 ID、就醫 ID、原檔列或偏移日期。**此勾選本身不授予資料使用權。**
...
2026-09-20：rag-only、single-agent、舊 multi-agent 與 v2 整合共用 demo 輸出容錯：整理額外欄位／空清單／重複項目，省略不適合展示的說明，逐項移除無法核對的藥品或引用。所有模式在沒有最終候選時，均以表格顯示「來源藥敏整理結果（demo）」。rule-only 不呼叫模型，來源判讀衝突、非 S、過敏等仍保留排除或待確認標示；不將未知結果補成敏感。JSON 無法解析、連線失敗或整體前置限制仍可能使模型結果未發布，但可查看來源整理表。

目前預設 v2.3 為寬鬆 demo：Agent 待確認與缺漏改為結果限制，不再直接停止整合；可修復的輸出格式自動容錯。前置檢查、藥品排除及引用邊界仍保留，無支持候選不補造。以下 v2.2 的重要待確認停止策略已由 v2.3 取代。

Demo v2.2：病例整理現在可讀取培養檢體、報告狀態與 AST。額外病史／影像未知列為限制；重要待確認事項不再阻止其他專責 Agent 分析，但仍暫緩整合與候選發布。既有混合病例可由根目錄執行 `.\backend\.venv\Scripts\python.exe .\backend\scripts\update_demo_context.py` 加入固定 demo 範圍說明，保留來源資料與歷史版本。

「藥品數量與去向」區分來源筆數、規則排除、最終選入及其餘未選入；例如 17 項來源可以是 6 項規則排除、4 項選入、7 項通過規則但未選入，並不表示另外 13 項都不適用。rag-only 的 AST 節點為程式規則核對，生成模型只收到摘要、測試藥品名稱與引用，沒有完整逐筆 AST。文件旁會標示合成測試／真實參考／來源類型未記錄；合成測試文件只驗證流程，不代表指引支持。
...
文件版本展開後可按「刪除此文件」，確認後移除該文件的原檔、片段、文字索引與向量。既有 run 的證據快照保留，但其原始文件連結不再可用；刪除不會改寫病例或自動替換指定引用。全部參考文件模式會在下次分析自動使用現存的參考文件；指定文件模式須自行改選。全庫搜尋仍受相關性、族群與版本衝突檢查限制，不代表每份文件都會被引用。

1. 開啟「文件資料庫」，選 PDF、UTF-8 `.md` 或 `.txt`，HTTP 上傳單檔上限 10 MiB（底層文件服務為 20 MiB）。
2. 輸入標題與版本，例如 `demo-policy-notes`／`v1`。
3. 人工核對文件後選擇適用族群：所有年齡、成人、兒童、混合或尚未標示。混合與未標示文件保留待確認；程式不會因出現「成人」等詞就認定適用。與病例族群不符或出現另一族群內容的片段會排除或暫緩。
4. 虛構文件勾選「虛構展示文件」，真實文件取消勾選。
5. 按「匯入並解析文件」，查看 processing_status、warnings、chunk_count。
6. 展開「文件版本」中的紀錄，查看 metadata 或開啟原檔。

`indexed` 表示解析切段完成，**不代表向量已建好**。同 hash 重複上傳回傳舊紀錄，不更新標題、版本或 synthetic 標記；不同內容另建文件。原始檔保存在本機 runtime。

既有文件可展開紀錄，在「此文件適用族群」選擇範圍，填寫「標示依據」（例如來源頁碼），按「儲存族群標示」。更新保存歷史與修訂號，不需重新建立向量。若別人已更新，會提示重新載入；重複上傳相同內容且指定不同族群會回報衝突，請使用此表單修改。

病例使用 reference 時必須有年齡；目前軟體以 18 歲區分成人與兒童，尚未細分新生兒等子族群。沒有相符且已標示的片段時不發布候選，也不呼叫生成模型。更新標示後請重新分析：舊 run 的快照不變；缺少族群標示的舊輸出不能直接通過新的審閱驗證。族群標示相符仍不表示片段支持某項藥物或臨床結論。
...
- **取消「虛構展示文件」勾選**。
- 同時涵蓋不同族群的全書請選「混合」。可先在文件庫搜尋、核對原頁；要供候選流程引用，需把適用段落整理為另份文件，保留原書名稱、版本、實體頁碼及章節，經人工核對後標示族群，再將該文件 doc_id 加入病例 policy_refs。不要把整本混合族群文件標成「所有年齡」來解除待確認。
...
在專案根目錄執行（需要原工作站保存的檔案）：

```powershell
.\backend\.venv\Scripts\python.exe .\backend\scripts\verify_live_workflow.py --inspect .\data\runtime\live-verification.json
```

`--inspect` 不載入模型設定、不啟動服務、不呼叫 API，只列出各模式的安全條件、錯誤代碼、候選／避免重疊數與引用快照是否存在。可指定匯出的 benchmark JSON；舊摘要沒有引用快照時會標示缺少，不能据此判斷臨床支持性。

只有確認要重新呼叫模型時才使用 `--live --policy-ref 文件ID`，並可加 `--diagnose` 保存原始模型回應。每次另建 `data/runtime/live-verifications/<唯一編號>/verification.json`，含引用快照與檢索／候選節點；原始回應分別保存為 `response-001.json` 等，不覆蓋舊診斷檔。任一模式有錯誤、候選與避免重疊，或未達可供審閱狀態，腳本會回報失敗。檢查通過仍不代表臨床結論正確。
