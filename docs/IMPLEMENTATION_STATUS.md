...
- 新增 2 個完整 synthetic 病例，總數 16；新增 `scripts/demo.sh` 明確使用 lexical＋mock，預先產生並保存結果。
...
- 後端：**81 passed、4 warnings**，coverage **86%**（RAG **84%**）；Python 3.14.5（Linux 轉換前的既有驗證紀錄）。
...
1. 在 Linux 安裝並啟動 [Ollama](https://ollama.com/download/linux)，執行 `ollama pull embeddinggemma`。
